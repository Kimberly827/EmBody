...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\louis\\OneDrive - The University of Melbourne\\Documents\\University Files\\Internship\\Working Pyschopy\\Click\\Click test_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    Intro_1 = visual.ImageStim(
        win=win,
        name='Intro_1', 
        image='Images/Intro1.png', mask=None, anchor='center',
    ...
    Intro_2 = visual.ImageStim(
        win=win,
        name='Intro_2', 
        image='Images/Intro2.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(1, 1),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ...
    Intro_3 = visual.ImageStim(
        win=win,
        name='Intro_3', 
        image='Images/Intro3.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), size=(1, 1),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ...
    lt_text_body = visual.TextStim(win=win, name='lt_text_body',
        text='for this body, color the regions\nwhose activity you feel \nincreasing or getting stronger',
        font='Arial',
        pos=(-0.33, -0.45), height=0.025, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-4.0);
    rt_text_body = visual.TextStim(win=win, name='rt_text_body',
        ...
        languageStyle='LTR',
        depth=-5.0);
    body_explanation = visual.TextStim(win=win, name='body_explanation',
        ...
        languageStyle='LTR',
        depth=-6.0);
    body_progress = visual.TextStim(win=win, name='body_progress',
        text='Any text\n\nincluding line breaks',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-7.0);
    reset_rect = visual.ShapeStim(
        win=win, name='reset_rect',
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        trialComponents = [target, mouse, key_resp_body, lt_text_body, rt_text_body, body_explanation, body_progress, reset_rect, feeling_words]
        ...
        while continueRoutine:
            ...
            buttons = mouse.getPressed()
            if buttons[0]:  # Left click (buttons[0] is left, buttons[1] is middle, buttons[2] is right)
                x, y = mouse.getPos()
                for shape in shapes:
                    if not shape.autoDraw:  # Find a shape that is not currently being drawn
                        shape.pos = (x, y)
                        shape.fillColor = 'red'  # Set the color for left click
                        shape.autoDraw = True  # Start drawing the shape
                ...
                for shape in shapes:
                    if not shape.autoDraw:  # Find a shape that is not currently being drawn
                        shape.pos = (x, y)
                        shape.fillColor = 'purple'  # Set the color for right click
                        shape.autoDraw = True  # Start drawing the shape
                        break  # Exit the loop after updating one shape
            ...
            if key_resp_body.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_body.getKeys(keyList=['space', 'return'], ignoreKeys=["escape"], waitRelease=False)
            ...
            # if lt_text_body is starting this frame...
            if lt_text_body.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                lt_text_body.frameNStart = frameN  # exact frame index
                lt_text_body.tStart = t  # local t and not account for scr refresh
                lt_text_body.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(lt_text_body, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'lt_text_body.started')
                # update status
                lt_text_body.status = STARTED
                lt_text_body.setAutoDraw(True)
            
            # if lt_text_body is active this frame...
            if lt_text_body.status == STARTED:
                # update params
                pass
            
            # *rt_text_body* updates
            
            # if rt_text_body is starting this frame...
            if rt_text_body.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                rt_text_body.frameNStart = frameN  # exact frame index
                rt_text_body.tStart = t  # local t and not account for scr refresh
                rt_text_body.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(rt_text_body, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'rt_text_body.started')
                # update status
                rt_text_body.status = STARTED
                rt_text_body.setAutoDraw(True)
            
            # if rt_text_body is active this frame...
            if rt_text_body.status == STARTED:
                # update params
                pass
            
            # *body_explanation* updates
            
            # if body_explanation is starting this frame...
            if body_explanation.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                body_explanation.frameNStart = frameN  # exact frame index
                body_explanation.tStart = t  # local t and not account for scr refresh
                body_explanation.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(body_explanation, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'body_explanation.started')
                # update status
                body_explanation.status = STARTED
                body_explanation.setAutoDraw(True)
            
            # if body_explanation is active this frame...
            if body_explanation.status == STARTED:
                # update params
                pass
            
            # if body_explanation is stopping this frame...
            if body_explanation.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > body_explanation.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    body_explanation.tStop = t  # not accounting for scr refresh
                    body_explanation.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'body_explanation.stopped')
                    # update status
                    body_explanation.status = FINISHED
                    body_explanation.setAutoDraw(False)
            
            # *body_progress* updates
            
            # if body_progress is starting this frame...
            if body_progress.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                body_progress.frameNStart = frameN  # exact frame index
                body_progress.tStart = t  # local t and not account for scr refresh
                body_progress.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(body_progress, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'body_progress.started')
                # update status
                body_progress.status = STARTED
                body_progress.setAutoDraw(True)
            
            # if body_progress is active this frame...
            if body_progress.status == STARTED:
                # update params
                pass
            
            # if body_progress is stopping this frame...
            if body_progress.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > body_progress.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    body_progress.tStop = t  # not accounting for scr refresh
                    body_progress.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'body_progress.stopped')
                    # update status
                    body_progress.status = FINISHED
                    body_progress.setAutoDraw(False)
            
            # *reset_rect* updates
            
            # if reset_rect is starting this frame...
            if reset_rect.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                reset_rect.frameNStart = frameN  # exact frame index
                reset_rect.tStart = t  # local t and not account for scr refresh
                reset_rect.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(reset_rect, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'reset_rect.started')
                # update status
                reset_rect.status = STARTED
                reset_rect.setAutoDraw(True)
            
            # if reset_rect is active this frame...
            if reset_rect.status == STARTED:
                # update params
                pass
            
            # if reset_rect is stopping this frame...
            if reset_rect.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > reset_rect.tStartRefresh + 1.0-frameTolerance:
                    # keep track of stop time/frame for later
                    reset_rect.tStop = t  # not accounting for scr refresh
                    reset_rect.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'reset_rect.stopped')
                    # update status
                    reset_rect.status = FINISHED
                    reset_rect.setAutoDraw(False)
