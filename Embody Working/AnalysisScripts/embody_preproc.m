function resmat=embody_preproc(data,base,base2,labels,mask,vis,savename)
...
%     close all
    ...
    % using image filter    
    for n=1:NC;
        if isnan(sum(data(n).paint))
            resmat(:,:,n)=zeros(522,171);
        else
...
% mkdir(['preprocessed/' subjects(s).name])
